...
while True:
    ...
    if len(name) <= 4:
        raise NameTooShortError('Mame must be more than 4 characters')
    ...
    if domain not in valid_domains:
        raise InvalidDomainError(f'{domain} is not valid domain. Domain must be one of the following: .com, .bg, .org, .net')
